...
const ANGLE_CHARACTERISTIC_UUID = 'beb5483e-36e1-4688-b7f5-ea07361b26c8';
...
export default class BLEBlock {
  async connect() {
    ...
    this.angleCharacteristic = await this.imuService.getCharacteristic(
      ANGLE_CHARACTERISTIC_UUID
    );
    console.log('AngleCharacteristic', this.angleCharacteristic.properties);
  ...
  async addListener(callback) {
    setInterval(async () => {
      ...
      const angleValue = await this.angleCharacteristic.readValue();

      const acc = this._readValueToPos(accValue);
      const gyro = this._readValueToPos(gyroValue);
      const angle = this._readValueToPos(angleValue);

      callback({ acc, gyro, angle });
      this.reading = false;
    }, 50);
  }
